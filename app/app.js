...
class Question {
  constructor(questionObj, id) {
    ...
    this.correct_answer = decodeHtml(questionObj.correct_answer);
  ...
  static getQuestions(category) {
    if (!category) {
      return fetch('https://opentdb.com/api.php?amount=10&type=multiple')
        ...
        .then((object) =>
          object.results.map((question, index) => new Question(question, index))
        )
        .catch((error) => console.log(error));
    } else {
      return fetch(
  ...
  static renderQuestion(theQuestion) {
    ...
    questionArea.innerHTML = theQuestion.question;
  }
  ...
  static renderQuestions(questions, id) {
    ...
    currentQ.incorrect_answers.forEach((answer) =>
      arrOfAnswers.push(new Answer(answer, false))
    );
...
class Answer {
  ...
  static render(answer1, answer2, answer3, answer4) {
    ...
    for (let i = 0; i < 4; i++) {
      arrOfChoices[i].innerText = arrOfAnswers[i];
    }
  }

  static checkAnswer(answer) {
    let deFormattedAnswer = answer;
    let theAnswer = theAnswers.find(
      (element) => element.answer === deFormattedAnswer
    );
    ...
    element.addEventListener('click', (e) => {
      if (answerChosen === false) {
        if (e.target.nodeName === 'BUTTON') {
          ...
          let isItTrue = Answer.checkAnswer(answer);
...
function showResult(isItTrue, theChoice) {
  if (isItTrue === true) {
    ...
    result.style.cssText = 'color: #6EFFA9; font-weight: 400 !important;';
  } else {
    theChoice.classList.add('red-btn');
    result.innerText = 'Wrong';
    result.style.cssText = 'color: #ff766e; font-weight: 400 !important;';
    showCurrectAnswer();
  }
}
...
function findCurrectAnswer() {
  ...
  theCurrectAnswer = theCurrectAnswer.answer;
...
function showCurrectAnswer() {
  ...
  let theCurrectChoice = allChoices.find(
    (element) => element.textContent === theCurrectAnswer
  );
